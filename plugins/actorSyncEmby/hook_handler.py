...
def handle_create_hook(
    ...
    if not performer:
        msg = f"找不到 ID 为 {performer_id} 的演员"
        log.error(msg)
        return msg
        ...
        if local_exporter:
            ...
            if export_func:
                export_func(
                    ...
                    export_mode=1,  # Hook 固定使用覆盖模式
                    server_conn=settings.get("server_connection", {}),
                    stash_api_key=settings.get("stash_api_key", ""),
                    dry_run=settings.get("dry_run", False)
                )
        log.info(f"[{PLUGIN_ID}] 已导出本地（hook_mode=1，覆盖模式）")
        msg = f"演员 {performer_id} 创建成功，已导出本地（覆盖模式）"
    
    # hook_mode=2：只上传 Emby（覆盖模式，异步）
    elif hook_mode == 2:
        if emby_uploader:
            # 启动 worker 异步上传
            ...
            log.info(f"[{PLUGIN_ID}] 已启动异步上传到 Emby（hook_mode=2，覆盖模式）")
        msg = f"演员 {performer_id} 创建成功，已启动异步上传 Emby（覆盖模式）"
    ...
    # hook_mode=3：同时输出本地 + Emby（都是覆盖模式）
        ...
        if emby_uploader:
            ...
            upload_settings["upload_mode"] = 1
            start_async_worker(performer_id, upload_settings)
        log.info(f"[{PLUGIN_ID}] 已同时导出本地和上传 Emby（hook_mode=3，都是覆盖模式）")
        msg = f"演员 {performer_id} 创建成功，已同时导出本地和上传 Emby（覆盖模式）"
...
def handle_update_hook(
    ...
    log.info(f"[{PLUGIN_ID}] Hook mode, processing single performer id={performer_id}（固定使用覆盖模式）")
    
    # 获取演员完整数据
    performer = stash.find_performer(performer_id)
    if not performer:
        msg = f"找不到演员 {performer_id}"
        log.error(msg)
        if task_log_func:
    ...
    if hook_mode == 1:
        ...
        msg = f"已导出演员 {performer_id} 到本地（hook_mode=1，覆盖模式）"
    ...
    # hook_mode=2：只上传 Emby（覆盖模式）
        ...
        msg = f"已上传演员 {performer_id} 到 Emby（hook_mode=2，覆盖模式）"
    ...
    # hook_mode=3：同时输出本地 + Emby（都是覆盖模式）
        ...
        msg = f"已同步演员 {performer_id} 到本地和 Emby（hook_mode=3，都是覆盖模式）"
